...
public class PayController {
    ...
    @RequestMapping("/pay")
    public Object pay() throws AlipayApiException, JsonProcessingException {
        AlipayTradePrecreateRequest request = new AlipayTradePrecreateRequest();//创建API对应的request类

        Map<String,String> map = new HashMap<>();
        map.put("out_trade_no","201911230204455007"); //商户订单号，需要保证不重复
        map.put("total_amount","19.56"); //订单金额
        map.put("subject","苹果手机"); //订单标题
        map.put("store_id","NJ_001"); //商户门店编号
        map.put("timeout_express","90m"); //订单允许的最晚付款时间

        ObjectMapper objectMapper = new ObjectMapper();
        String string = objectMapper.writeValueAsString(map);

        request.setBizContent(string);
        request.setNotifyUrl("444444");
